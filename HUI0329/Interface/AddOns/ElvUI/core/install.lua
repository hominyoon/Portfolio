...
local function SetupAuraWatch(toggle)
	if toggle then 
		...
		E:Print(L["One or more of the changes you have made require a ReloadUI."]);
	else
		E.db.unitframe.units['player']['aurabar'].enable = false
		E.db.unitframe.units['target']['aurabar'].enable = false
		E.db.unitframe.units['focus']['aurabar'].enable = false
		E.private.AuraWatch.enable = true;
		E:Print(L["One or more of the changes you have made require a ReloadUI."]);
	end
	InstallStepComplete.message = L["Auras Set"]
	InstallSetpComplete:Show()	
end
...
function E:SetupResolution(noDataReset)
	...
	if self == 'low' then
		...
		if not noDataReset then
			E.db.chat.panelWidth = 362
			E.db.chat.panelHeight = 180
			E.db.chat.panelWidthRight = 362
...
local function SetPage(PageNum)
	...
	elseif PageNum == 4 then
		...
		InstallOption3Button:SetText(L['Dark'])
	...
	elseif PageNum == 7 then
		f.SubTitle:SetText(L['Raid Layout'])
		f.Desc1:SetText(L['Set RaidFrame Style'])
		f.Desc2:SetText(L['CENTER, SINGLE, RIGHT, DEFAULT'])
		f.Desc3:SetText(L["Importance: |cffD3CF00Medium|r"])
		InstallOption1Button:Show()
		InstallOption1Button:SetScript('OnClick', function() E:SetupEuiLayout('CENTER') end)
		InstallOption1Button:SetText(L['Center Style'])
		InstallOption2Button:Show()
		InstallOption2Button:SetScript('OnClick', function() E:SetupEuiLayout('SINGLE') end)
		InstallOption2Button:SetText(L['Single Style'])
		InstallOption3Button:Show()
		InstallOption3Button:SetScript('OnClick', function() E:SetupEuiLayout('RIGHT') end)
		InstallOption3Button:SetText(L['Right Style'])
		InstallOption4Button:Show()
		InstallOption4Button:SetScript('OnClick', function() E:SetupEuiLayout('DEFAULT') end)
		InstallOption4Button:SetText(L['Default Style'])
	...
	elseif PageNum == 10 then	
		...
		f.Desc3:SetText(L["Aura Watch module"])
		InstallOption1Button:Show()
		InstallOption1Button:SetScript('OnClick', function() SetupAuras('classic'); SetupAuraWatch(true); end)
		InstallOption1Button:SetText(L['Classic'])
		InstallOption2Button:Show()
		InstallOption2Button:SetScript('OnClick', function() SetupAuras() end)
		InstallOption2Button:SetText(L['Icons Only'])
		InstallOption3Button:Show()
		InstallOption3Button:SetScript('OnClick', function() SetupAuraWatch() end)
		InstallOption3Button:SetText(L["AuraWatch"])
