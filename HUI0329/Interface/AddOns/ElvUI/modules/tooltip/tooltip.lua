...
function TT:GameTooltip_OnTooltipSetUnit(tt)
	...
	if(UnitIsPlayer(unit)) then
		...
		if(levelLine) then
			...
			levelLine:SetFormattedText("|cff%02x%02x%02x%s|r %s |c%s%s|r", diffColor.r * 255, diffColor.g * 255, diffColor.b * 255, level > 0 and level or "??", race or UNKNOWN, color.colorStr, localeClass or UNKNOWN)
		end

		--High CPU usage, restricting it to shift key down only.
	--	if(self.db.inspectInfo and isShiftKeyDown) then
		if self.db.inspectInfo then --by eui.cc
		...
		if(UnitIsTapped(unit) and not UnitIsTappedByPlayer(unit)) then
			...
			local UReaction = UnitReaction(unit, "player");
			if UReaction then
				color = E.db.tooltip.useCustomFactionColors and E.db.tooltip.factionColors[""..UReaction] or FACTION_BAR_COLORS[UReaction]
			else
