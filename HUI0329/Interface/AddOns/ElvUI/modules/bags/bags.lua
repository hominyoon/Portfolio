...
local ITEMTYPE1, ITEMTYPE2 = GetAuctionItemClasses()
...
function B:UpdateSlot(bagID, slotID)
	...
	elseif (clink) then
		local iType;
		slot.name, _, slot.rarity, slot.itemLevel, _, iType = GetItemInfo(clink);
		local itemId = tonumber(clink:match("item:(%d+)"))
		...
		elseif slot.rarity and slot.rarity > 1 then
			...
			if slot.itemLevel and slot.rarity ~= 7 and slot.itemLevel < E.db.bags.itemLevelMark and E.mylevel == 100 and ((iType == ITEMTYPE1) or (iType == ITEMTYPE2)) then
				slot.itemLevelText:SetText(E.db.bags.itemLevelFormat == 'number' and slot.itemLevel or '*')
			else
		...
		if numSlots > 0 then
			...
			for slotID = 1, numSlots do
				...
				if not f.Bags[bagID][slotID] then
					...
					if (not f.Bags[bagID][slotID].questIcon) and _G[f.Bags[bagID][slotID]:GetName()..'IconQuestTexture'] then
					...
					f.Bags[bagID][slotID].itemLevelText = f.Bags[bagID][slotID]:CreateFontString(nil,"OVERLAY")
					f.Bags[bagID][slotID].itemLevelText:FontTemplate(nil, 12, 'OUTLINE')
					f.Bags[bagID][slotID].itemLevelText:SetPoint("TOPRIGHT")
					f.Bags[bagID][slotID].itemLevelText:SetJustifyH("CENTER")
					f.Bags[bagID][slotID].itemLevelText:SetTextColor(0.61,0.61,0.61)
				end
