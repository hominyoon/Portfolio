...
function RU:Initialize()
	if E.db['datatexts']['panels']['TopDataTextsBar3'].left ~= '' or E.db['datatexts']['panels']['TopDataTextsBar3'].right ~= '' or E.db['datatexts']['panels']['TopDataTextsBar3'].middle ~= '' then return; end
	...
	RaidUtility_ShowButton:SetScript("OnEnter", function(self)
		GameTooltip:SetOwner(self, 'ANCHOR_CURSOR_RIGHT', 24, -40)
		GameTooltip:AddDoubleLine(L['Right Click:'], L["Move it"]);
		GameTooltip:Show()
	end)
	
	RaidUtility_ShowButton:SetScript("OnLeave", function(self)
		GameTooltip:Hide()
	end)
