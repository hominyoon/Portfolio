...
function NP:SetTargetIndicator()
	...
	elseif(self.db.targetIndicator.style == 'doubleArrow' or self.db.targetIndicator.style == 'doubleArrowInverted' or self.db.targetIndicator.style == 'doubleTPArrow') then
		...
		if self.db.targetIndicator.style == 'doubleTPArrow' then
			targetIndicator.left:SetTexture([[Interface\AddOns\ElvUI\media\textures\nameplate_arrows_left.tga]])
			targetIndicator.right:SetTexture([[Interface\AddOns\ElvUI\media\textures\nameplate_arrows_right.tga]])
		else
...
function NP:CheckFilterAndHealers(myPlate)
	...
	if NP.Class[name] and NP.db.classIcon.enable then
		local tCoords = CLASS_BUTTONS[NP.Class[name]]
		myPlate.classIcon:SetTexCoord(tCoords[1], tCoords[2], tCoords[3], tCoords[4])
		myPlate.classIcon:Show()
	else
		myPlate.classIcon:Hide()
	end
...
function NP:CheckBGHealers()
	local name, _, talentSpec, class
	for i = 1, GetNumBattlefieldScores() do
		name, _, _, _, _, _, _, _, class, _, _, _, _, _, _, talentSpec = GetBattlefieldScore(i);
		if name then
...
function NP:CheckArenaHealers()
	...
	for i=1, 5 do
		...
		if name and name ~= UNKNOWN then
			...
			local class
			if s and s > 0 then
				_, talentSpec, _, _, _, _, class = GetSpecializationInfoByID(s)
			end
...
function NP:OnHide()
	...
	if myPlate.healthBar and NP.db.healthBar then
...
function NP:SizeAuraHeader(myPlate, width, auraHeader, dbTable)
	...
	if perRow > numAuras then perRow = numAuras; end
--	local auraWidth = ((width - (baseSpacing * (numAuras - 1))) / numAuras)
	local auraWidth = ((width - (baseSpacing * (perRow - 1))) / perRow)
	...
	for index = 1, numAuras do
		...
		if(dbTable == 'debuffs') then --œ‘ æ¡Ω––buff/debuff --by eui.cc
			if(index == 1) then
			...
			elseif index <= perRow then
				...
				auraHeader.icons[index]:SetPoint("BOTTOMLEFT", auraHeader.icons[index-perRow], "TOPLEFT", 0, baseSpacing)
			end
		else
			if(index == 1) then
			...
			elseif index <= perRow then
				...
				auraHeader.icons[index]:SetPoint("BOTTOMRIGHT", auraHeader.icons[index-perRow], "TOPLEFT", 0, baseSpacing)
			end
		end
	end
...
function NP:UpdateSettings()
	...
	--Class Icon
	myPlate.classIcon:ClearAllPoints()
	myPlate.classIcon:SetPoint(E.InversePoints[NP.db.classIcon.attachTo], myPlate.healthBar, NP.db.classIcon.attachTo, NP.db.classIcon.xOffset, NP.db.classIcon.yOffset)
	myPlate.classIcon:SetSize(NP.db.classIcon.size, NP.db.classIcon.size)
...
function NP:CreatePlate(frame)
	...
	--Class Icon
	myPlate.classIcon = myPlate:CreateTexture(nil, 'ARTWORK')
	myPlate.classIcon:SetSize(frame.raidIcon:GetSize())
	myPlate.classIcon:SetTexture([[Interface\WorldStateFrame\Icons-Classes]]);
	myPlate.classIcon:Hide()
...
function NP:CreateAuraIcon(frame, parent, dbTable)
	...
	button.TimeLeft:Point('TOPLEFT', db.timeX, db.timeY)
	...
	button.Stacks:SetPoint("BOTTOMRIGHT", button, "BOTTOMRIGHT", db.stacksX, db.stacksY)
...
--		local auraID = spellID.."UNKNOWN_CASTER"
--		if NP.AuraList[guid][spellID..tostring(UnitGUID('player'))] then auraID = spellID..tostring(UnitGUID('player')) end
...
function NP:SetAuraInstance(guid, spellID, expiration, stacks, caster, duration, texture, auraType, auraTarget)
	...
	local trackFilter2 = E.global['unitframe']['aurafilters'][db.additionalFilter2]
	if db.additionalFilter2 and trackFilter2 then
		local name = GetSpellInfo(spellID)
		local spellList = trackFilter2.spells
		local type = trackFilter2.type
