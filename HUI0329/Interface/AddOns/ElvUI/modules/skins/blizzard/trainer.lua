...
local function LoadSkin()
	...
	local trainAll = CreateFrame("Button", "ClassTrainerTrainAllButton", ClassTrainerFrame, "UIPanelButtonTemplate")
	trainAll:SetText(QUICKBUTTON_NAME_EVERYTHING)
	trainAll:SetPoint("TOPRIGHT", ClassTrainerTrainButton, "TOPLEFT", -4, 0)
	trainAll:StripTextures()
	S:HandleButton(trainAll)
	trainAll:SetWidth(min(50, trainAll:GetTextWidth() + 15))
	
	hooksecurefunc("ClassTrainerFrame_Update",function()
		local count=GetNumTrainerServices();
		for i=1,count do
			if ClassTrainerTrainAllButton:IsEnabled() and select(3,GetTrainerServiceInfo(i))=="available" then
	...
	trainAll:SetScript("OnClick", function(self)
		local count=GetNumTrainerServices();
		for i=1,count do
