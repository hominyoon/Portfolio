...
function CH:StyleChat(frame)
	...
	if a and b and c then a:Kill(); b:Kill(); c:Kill(); end;
...
function CH:FindHylink(msg, keyword) --by eui.cc
	local maxNum = string.len(msg)
	if maxNum < 2 then return msg end
	local i = 1
	local message = ''
	if not string.find(msg, '|c') then return msg:gsub(keyword, E.media.hexvaluecolor..keyword..'|r') end
	
	repeat
		msg = msg:sub(i, -1)
		local pos1 = string.find(msg, '|c')
		local pos2 = string.find(msg, '|r')
		if pos1 and pos2 and pos2 > pos1 then
			local msg1, msg2, msg3 = msg:sub(1, pos1-1), msg:sub(pos1, pos2+1), msg:sub(pos2+2, -1)
			if msg1:find(keyword) then msg1 = msg1:gsub(keyword, E.media.hexvaluecolor..keyword..'|r') end
			if msg3:find(keyword) then
				if not msg3:find('|c') then
					msg3 = msg3:gsub(keyword, E.media.hexvaluecolor..keyword..'|r')
					return message..msg1..msg2..msg3
				else
...
function CH:CheckKeyword(message)
	...
	for keyword, _ in pairs(CH.Keywords) do
		...
		--	message = string.gsub(message, keyword, E.media.hexvaluecolor..keyword..'|r')
			message = CH:FindHylink(message, keyword)
			if self.db.keywordSound ~= 'None' and not self.SoundPlayed  then
				PlaySoundFile(LSM:Fetch("sound", self.db.keywordSound), "Master")
				self.SoundPlayed = true
				self.SoundTimer = CH:ScheduleTimer('ThrottleSound', 1)			
...
--join BigFoot Channel
local joinChannelFunc = function(channel)
	if E.db.chat.autojoin ~= true then return end
...
function CH:CHANNEL_PASSWORD_REQUEST(...)
	if self.db.autojoin ~= true then return end
    local _,channelName = ... 
    if channelName:find(L["BigFootChannel"]) then 
        self.nextChannel = getNextChannel(channelName)
        joinChannelFunc(self.nextChannel) 
    else
        local dialog = E:StaticPopup_Show("CHAT_CHANNEL_PASSWORD", channelName);
        if ( dialog ) then 
...
function CH:Initialize()
	...
	GeneralDockManagerOverflowButton:Point('BOTTOMRIGHT', LeftChatTab, 'BOTTOMRIGHT', -2, 4)
