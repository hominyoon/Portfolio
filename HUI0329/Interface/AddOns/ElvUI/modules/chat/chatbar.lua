...
local ChannelList = {
	[L["BigFootChannel"]] = string.sub(L["BigFootChannel"],7,9),
}

local function ChannelShortText(index)
	local channelNum, channelName = GetChannelName(index);
	
	if channelNum ~= 0 and channelName ~= 'Collection of stone' then
...
for k, v in pairs(ChatTypesList) do
	table.insert(chatTypesList, k)
end
table.sort(chatTypesList, function(a, b)
	return a < b
end)
...
local function ChannelButton(parent, width, height, postion, order, text, color)
	...
	if E.kolocale and text == E:ShortenString(L["BigFootChannel"],1) then text = ChannelList[L["BigFootChannel"]]; color = {23/255, 132/255, 209/255}; end
