...
function UF:UpdateColors()
	...
	ElvUF.colors.Maelstrom = {}
	ElvUF.colors.Maelstrom[1] = E:GetColorTable(db.classResources.SHAMAN[1])
	ElvUF.colors.Maelstrom[2] = E:GetColorTable(db.classResources.SHAMAN[2])
	ElvUF.colors.Maelstrom[3] = E:GetColorTable(db.classResources.SHAMAN[3])
	ElvUF.colors.Maelstrom[4] = E:GetColorTable(db.classResources.SHAMAN[4])
	ElvUF.colors.Maelstrom[5] = E:GetColorTable(db.classResources.SHAMAN[5])	
	...
	if(raidFilter and numGroups) then
		...
		if(inInstance and (instanceType == 'raid' or instanceType == 'pvp')) then
			...
			if maxPlayers > 0 then
				...
				E:Print(L["Forcing maxGroups to: %d because maxPlayers is: %d"]:format(numGroups,maxPlayers))
			end
		end
	end
