...
local CAN_HAVE_CLASSBAR = (E.myclass == "SHAMAN" or E.myclass == "PALADIN" or E.myclass == "DRUID" or E.myclass == "DEATHKNIGHT" or E.myclass == "WARLOCK" or E.myclass == "PRIEST" or E.myclass == "MONK" or E.myclass == 'MAGE' or E.myclass == 'ROGUE')
...
function UF:Construct_PlayerFrame(frame)
	...
	frame.Health = self:Construct_HealthBar(frame, true, true, 'RIGHT', E.db.unitframe.units.player.health.orientation)
	frame.Health.frequentUpdates = true;
	
	frame.Power = self:Construct_PowerBar(frame, true, true, 'LEFT', E.db.unitframe.units.player.power.orientation)
	...
	frame.GCD = self:ConstructGCDBar(frame)
	...
	elseif E.myclass == 'SHAMAN' then
		frame.Maelstrom = self:Construct_MaelstromBar(frame)
		frame.ClassBar = 'Maelstrom'
	end
	...
	E:CreateMover(frame, frame:GetName()..'Mover', L['Player Frame'], nil, nil, nil, 'ALL,SOLO', function() return E.db.unitframe.units.player.enable; end)
	if not E.db['unitframe']['units'].player.aurabar.lock then
		E:CreateMover(frame.AuraBars.Holder, frame:GetName()..'AuraMover',  L['Player Frame'].. L['Aura Bars'], nil, nil, nil, 'ALL,SOLO', function() return E.db.unitframe.units.player.aurabar.enable; end)
	end
end
...
--	if InCombatLockdown() then return; end
	...
	if power.tick1 then
		local tickHeight = db.power.height
		if USE_POWERBAR_OFFSET then
			tickHeight = power:GetHeight()
		end
		power.tick1:SetHeight(tickHeight*1.5)
		power.tick2:SetHeight(tickHeight*1.5)
		power.tick3:SetHeight(tickHeight*1.5)
		
		power.tick1:ClearAllPoints()
		if db.power.tick1 > 0 then
			power.tick1:SetPoint("LEFT", power, "LEFT", power:GetWidth()*db.power.tick1, 0)
		end
		power.tick3:ClearAllPoints()
		if db.power.tick3 > 0 then
			power.tick3:SetPoint("LEFT", power, "LEFT", power:GetWidth()*db.power.tick3, 0)
		end
		power.tick2:ClearAllPoints()
		if db.power.tick2 > 0 then
			power.tick2:SetPoint("LEFT", power, "LEFT", power:GetWidth()*db.power.tick2, 0)
		end
	end	
end
		...
		if USE_POWERBAR then
			...
			if POWERBAR_DETACHED then
				...
				if not power.mover then
					...
					E:CreateMover(power, 'PlayerPowerBarMover', L['Player Powerbar'], nil, nil, nil, 'ALL,SOLO', function() return E.db.unitframe.units.player.power.detachFromFrame; end)
				else
					power:ClearAllPoints()
		...
		--	end			
			...
			elseif not db.classbar.detachFromFrame then
				...
				if not bars.mover then
					...
					E:CreateMover(bars, 'ClassBarMover', L['Classbar'], nil, nil, nil, 'ALL,SOLO', function() return E.db.unitframe.units.player.classbar.detachFromFrame; end)
				else
					bars:ClearAllPoints()
...
--[[ 	--Stagger
	...
	do
		...
		if db.aurabar.enable then
			...
			if db.aurabar.lock then
				auraBars:SetPoint(anchorPoint..'LEFT', attachTo, anchorTo..'LEFT', (attachTo == frame and anchorTo == 'BOTTOM') and POWERBAR_OFFSET or 0, E.PixelMode and -1 or yOffset)
			...
			elseif db.aurabar.lock == false and auraBars.Holder then
				...
				auraBars.Holder:SetHeight(db.aurabar.height)
				auraBars:SetPoint(anchorPoint..'LEFT', auraBars.Holder, anchorTo..'LEFT', 0, E.PixelMode and -1 or yOffset)
				auraBars:SetPoint(anchorPoint..'RIGHT', auraBars.Holder, anchorTo..'RIGHT', -POWERBAR_OFFSET, E.PixelMode and -1 or yOffset)
			end
	...
	do
		...
		if db.Swing.enable then
			...
			swing:Width(db.Swing.width - (E.PixelMode and 2 or (BORDER * 2)))
			swing:Height(db.Swing.height)
			swing.Holder:Width(db.Swing.width + (E.PixelMode and 0 or (BORDER * 2)))
			swing.Holder:Height(db.Swing.height + (E.PixelMode and 2 or (BORDER * 2)))
	...
	do
		...
		if db.gcd then
			...
			bar:SetHeight(4)
			bar:SetPoint("BOTTOMLEFT", frame, "TOPLEFT", 0, 0)
		else
	...
	do
		if CAN_HAVE_TANKSHIELD then
			...
			if db.tankshield.enable then
				...
				local pOffset = 0
				if USE_POWERBAR_OFFSET then pOffset = db.power.offset end
				
				TankShield:ClearAllPoints()
				TankShield.sb:ClearAllPoints()				
				if db.tankshield.sizeOverride == 0 then
					TankShield:Point("TOPLEFT", frame.Health.backdrop, "TOPRIGHT", SPACING + pOffset + db.tankshield.xOffset, db.tankshield.yOffset)
					if USE_INSET_POWERBAR or USE_MINI_POWERBAR then
						TankShield:Point("BOTTOMRIGHT", frame.Health.backdrop, "BOTTOMRIGHT", SPACING + UNIT_HEIGHT - POWERBAR_HEIGHT + db.tankshield.xOffset,  db.tankshield.yOffset)
					elseif USE_POWERBAR_OFFSET then
						TankShield:Point("BOTTOMRIGHT", frame.Health.backdrop, "BOTTOMRIGHT", SPACING + UNIT_HEIGHT + pOffset + db.tankshield.xOffset, -pOffset + db.tankshield.yOffset)
					elseif POWERBAR_DETACHED then
						TankShield:Point("BOTTOMRIGHT", frame.Health.backdrop, "BOTTOMRIGHT", SPACING + UNIT_HEIGHT + db.tankshield.xOffset, db.tankshield.yOffset)
					else
						TankShield:Point("BOTTOMRIGHT", frame.Power.backdrop, "BOTTOMRIGHT", SPACING + UNIT_HEIGHT - pOffset + db.tankshield.xOffset, db.tankshield.yOffset)
					end
				else
					TankShield:Point("TOPLEFT", frame.Health.backdrop, "TOPRIGHT", SPACING + pOffset + db.tankshield.xOffset, db.tankshield.yOffset)
