...
function UF:Construct_FocusFrame(frame)	
	frame.Health = self:Construct_HealthBar(frame, true, true, 'RIGHT', E.db.unitframe.units.focus.health.orientation)
	frame.Health.frequentUpdates = true;
	
	frame.Power = self:Construct_PowerBar(frame, true, true, 'LEFT', E.db.unitframe.units.focus.power.orientation)
	
	frame.Name = self:Construct_NameText(frame)
	
	frame.Buffs = self:Construct_Buffs(frame)
	frame.RangeText = self:ConstructFocusRange(frame)
	...
	E:CreateMover(frame, frame:GetName()..'Mover', L['Focus Frame'], nil, nil, nil, 'ALL,SOLO', function() return E.db.unitframe.units.focus.enable; end)
	if not E.db['unitframe']['units'].focus.aurabar.lock then
		E:CreateMover(frame.AuraBars.Holder, frame:GetName()..'AuraMover',  L['Focus Frame'].. L['Aura Bars'], nil, nil, nil, 'ALL,SOLO', function() return E.db.unitframe.units.focus.aurabar.enable; end)
	end
end
	...
	do
		...
		if db.aurabar.enable then
			...
			elseif db.aurabar.lock == false and auraBars.Holder then
				...
				auraBars:SetPoint(anchorPoint..'LEFT', auraBars.Holder, anchorTo..'LEFT', POWERBAR_OFFSET, yOffset)
				auraBars:SetPoint(anchorPoint..'RIGHT', auraBars.Holder, anchorTo..'RIGHT', -POWERBAR_OFFSET, yOffset)			
			end
