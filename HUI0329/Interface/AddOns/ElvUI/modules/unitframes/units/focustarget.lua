...
function UF:Construct_FocusTargetFrame(frame)	
	frame.Health = self:Construct_HealthBar(frame, true, true, 'RIGHT', E.db.unitframe.units.focustarget.health.orientation)
	
	frame.Power = self:Construct_PowerBar(frame, true, true, 'LEFT', E.db.unitframe.units.focustarget.power.orientation)
	...
	E:CreateMover(frame, frame:GetName()..'Mover', L['FocusTarget Frame'], nil, -7, nil, 'ALL,SOLO', function() return E.db.unitframe.units.focustarget.enable; end)
end
