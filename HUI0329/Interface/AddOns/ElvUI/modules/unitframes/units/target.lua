...
function UF:Construct_TargetFrame(frame)	
	frame.Health = self:Construct_HealthBar(frame, true, true, 'RIGHT', E.db.unitframe.units.target.health.orientation)
	frame.Health.frequentUpdates = true;
	
	frame.Power = self:Construct_PowerBar(frame, true, true, 'LEFT', E.db.unitframe.units.target.power.orientation)
	...
	frame.RangeText = self:ConstructTargetRange(frame)
	...
	frame.FactionIcon = self:Construct_FactionIcon(frame)
	
	frame:RegisterEvent('PLAYER_TARGET_CHANGED', UF.UpdateTargetRare)
	tinsert(frame.__elements, UF.UpdateTargetRare)
	...
	E:CreateMover(frame, frame:GetName()..'Mover', L['Target Frame'], nil, nil, nil, 'ALL,SOLO', function() return E.db.unitframe.units.target.enable; end)
	if not E.db['unitframe']['units'].target.aurabar.lock then
		E:CreateMover(frame.AuraBars.Holder, frame:GetName()..'AuraMover',  L['Target Frame'].. L['Aura Bars'], nil, nil, nil, 'ALL,SOLO', function() return E.db.unitframe.units.target.aurabar.enable; end)
	end
end
...
function UF:Update_TargetFrame(frame, db)
		...
		if USE_POWERBAR then
			...
			if POWERBAR_DETACHED then
				...
				if not power.mover then
					...
					E:CreateMover(power, 'TargetPowerBarMover', 'Target Powerbar', nil, nil, nil, 'ALL,SOLO', function() return E.db.unitframe.units.target.power.detachFromFrame; end)
				else
					power:ClearAllPoints()
	...
	--FactionIcon
	do	
		local factionIcon = frame.FactionIcon

		factionIcon:ClearAllPoints()
		factionIcon:Point('CENTER', frame, 'CENTER', db.factionIcon.xOffset, db.factionIcon.yOffset)
		factionIcon:SetSize(db.factionIcon.size, db.factionIcon.size)
		
		if db.factionIcon.enable and not frame:IsElementEnabled('FactionIcon') then
			frame:EnableElement('FactionIcon')
		elseif not db.factionIcon.enable and frame:IsElementEnabled('FactionIcon') then
			frame:DisableElement('FactionIcon')	
		end					
	end
	...
	do
		...
		if db.DragonOverlayStyle == 'classic' then
			rare:SetSize(256, 128)
			rare:SetPoint('TOPRIGHT', frame, 'TOPRIGHT', 100, 15)
			rare:Show()
		elseif db.DragonOverlayStyle == 'modern' then
			rare:SetSize(100, 100)
			rare:SetPoint('TOPRIGHT', frame, 'TOPRIGHT', 36, 8)
			rare:Show()
		else
			rare:Hide()
		end
