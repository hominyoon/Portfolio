...
function UF:Construct_PetTargetFrame(frame)	
	frame.Health = self:Construct_HealthBar(frame, true, true, 'RIGHT', E.db.unitframe.units.pettarget.health.orientation)
	
	frame.Power = self:Construct_PowerBar(frame, true, true, 'LEFT', E.db.unitframe.units.pettarget.power.orientation)
	...
	E:CreateMover(frame, frame:GetName()..'Mover', L['PetTarget Frame'], nil, -7, nil, 'ALL,SOLO', function() return E.db.unitframe.units.pettarget.enable; end)
end
