...
ElvUF.Tags.OnUpdateThrottle['distance165595'] = 0.1
ElvUF.Tags.Methods['distance165595'] = function(unit)
	local d
	if UnitIsConnected(unit) and not UnitIsUnit(unit, 'player') then
		local name,_,icon,count = UnitDebuff(GetSpellInfo(165595))
		if not name then
			name,_,icon,count = UnitDebuff(GetSpellInfo(178543))
		end
		if not name or (count and count < 4) then return '' end
		
		d = E:GetDistance('player', unit)
		if d and (type(d) == 'number') and d <30 then
			return L['Bad']
		end
	end
end