local E, L, V, P, G= unpack(select(2, ...)); --Inport: Engine, Locales, PrivateDB, ProfileDB, GlobalDB, Localize Underscore
...
local function UpdateFillBar(frame, previousTexture, bar, amount)
	...
	if orientation == 'HORIZONTAL' then
		if E.db.unitframe.transparent and E.db.general.transparentStyle == 2 then
			...
			bar:SetPoint('BOTTOMLEFT', frame.Health:GetStatusBarTexture(), 'BOTTOMRIGHT', w, 0)
			bar:SetPoint('TOPLEFT', frame.Health:GetStatusBarTexture(), 'TOPRIGHT', w, 0)
		else
