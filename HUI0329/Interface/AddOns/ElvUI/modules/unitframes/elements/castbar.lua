...
function UF:CustomCastDelayText(duration)
	...
	local timeformat = "%."..tostring(E.db.unitframe.casttimeformat).."f"
	
	if self.channeling then
		if db.castbar.format == 'CURRENT' then
			self.Time:SetText((timeformat.." |cffaf5050%.1f|r"):format(abs(duration - self.max), self.delay))
		elseif db.castbar.format == 'CURRENTMAX' then
			self.Time:SetText((timeformat.." / "..timeformat.." |cffaf5050%.1f|r"):format(duration, self.max, self.delay))
		elseif db.castbar.format == 'REMAINING' then
			self.Time:SetText((timeformat.." |cffaf5050%.1f|r"):format(duration, self.delay))
		end			
	else
		if db.castbar.format == 'CURRENT' then
			self.Time:SetText((timeformat.." |cffaf5050%s %.1f|r"):format(duration, "+", self.delay))
		elseif db.castbar.format == 'CURRENTMAX' then
			self.Time:SetText((timeformat.." / "..timeformat.." |cffaf5050%s %.1f|r"):format(duration, self.max, "+", self.delay))
		elseif db.castbar.format == 'REMAINING' then
			self.Time:SetText((timeformat.." |cffaf5050%s %.1f|r"):format(abs(duration - self.max), "+", self.delay))
		end		
	end
end
...
function UF:CustomTimeText(duration)
	local db = self:GetParent().db
	if not db then return end
	local timeformat = "%."..E.db.unitframe.casttimeformat.."f"
	
	if self.channeling then
		if db.castbar.format == 'CURRENT' then
			self.Time:SetText((timeformat):format(abs(duration - self.max)))
		elseif db.castbar.format == 'CURRENTMAX' then
			self.Time:SetText((timeformat.." / "..timeformat):format(duration, self.max))
			self.Time:SetText((timeformat.." / "..timeformat):format(abs(duration - self.max), self.max))
		elseif db.castbar.format == 'REMAINING' then
			self.Time:SetText((timeformat):format(duration))
		end				
	else
		if db.castbar.format == 'CURRENT' then
			self.Time:SetText((timeformat):format(duration))
...
function UF:PostCastStart(unit, name, rank, castid)
	...
	if not self.interrupt and unit == "target" and db.castbar.InterruptSound and CanInterrupt() and UnitCanAttack("player", unit) then
		PlaySoundFile(UF.LSM:Fetch("sound", "Kick Cast"), "Master")
	end
	self.Casting = true	
end
