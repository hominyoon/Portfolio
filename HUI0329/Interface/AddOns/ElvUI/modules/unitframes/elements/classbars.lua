...
--SHAMAN
function UF:Construct_MaelstromBar(frame)
	local bars = CreateFrame("Frame", nil, frame)
	bars:CreateBackdrop((E.db.general.transparent and E.db.general.transparentStyle == 1) and 'notrans' or 'Default')
	...
	for i = 1, UF['classMaxResourceBar'][E.myclass] do					
		...
		bars[i]:CreateBackdrop((E.db.general.transparent and E.db.general.transparentStyle == 1) and 'notrans' or 'Default')
		bars[i].backdrop:SetParent(bars)
	end
	
	bars.PostUpdate = UF.UpdateMaelstromCharges
	
	return bars
end

function UF:UpdateMaelstromCharges(event)
...
function UF:Construct_Stagger(frame)
	...
	stagger:CreateBackdrop((E.db.general.transparent and E.db.general.transparentStyle == 1) and 'notrans' or 'Default')
...
function UF:UpdateShardBar(spec)
	...
	local SPACING = db.classbar.fill == 'spaced' and 11 or 3
	for i = 1, maxBars do
		self[i]:SetHeight(self:GetHeight())	
...
function UF:Construct_DeathKnightResourceBar(frame)
	...
	runes:CreateBackdrop((E.db.general.transparent and E.db.general.transparentStyle == 1) and 'notrans' or 'Default')
	...
	for i = 1, UF['classMaxResourceBar'][E.myclass] do
		...
		runes[i]:CreateBackdrop((E.db.general.transparent and E.db.general.transparentStyle == 1) and 'notrans' or 'Default')
		...
		runes[i].PostUpdateRune = UF.PostUpdateRune
	end
...
function UF:Construct_DruidResourceBar(frame)
	...
	eclipseBar:CreateBackdrop((E.db.general.transparent and E.db.general.transparentStyle == 1) and 'notrans' or 'Default')
	...
	eclipseBar.Text:FontTemplate(nil, 12)
...
function UF:PostUpdateRune(rid)
	if not E.db.unitframe.units.player.classbar.dkRunesBorderColor then return; end

	local start, duration, runeReady = GetRuneCooldown(rid)
	if runeReady then
		self.backdrop:SetBackdropBorderColor(self:GetStatusBarColor())
	else
		self.backdrop:SetBackdropBorderColor(E['media'].bordercolor)
	end
end
