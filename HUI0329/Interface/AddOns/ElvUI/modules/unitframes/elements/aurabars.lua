...
function UF:Construct_AuraBars()
	...
	bar.bg:Hide()
	if E.db.general.transparent and E.db.general.transparentStyle == 1 then bar.bg:SetAlpha(E.db.general.backdropfadecolor.a or 0.4) end
...
function UF:AuraBarFilter(unit, name, rank, icon, count, debuffType, duration, expirationTime, unitCaster, isStealable, shouldConsolidate, spellID)
	...
	if UF:CheckFilter(db.noDuration, isFriend) then
		if (duration == 0 or not duration) and (count == 0 or not count) then --过滤无持续时间的光环,但显示有多层效果的无持续时间光环 by eui.cc
			returnValue = false;
		end
