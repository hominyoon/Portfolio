...
function UF:Construct_RaidFrames(unitGroup)
	...
	self.Health = UF:Construct_HealthBar(self, true, true, 'RIGHT', E.db.unitframe.units['raid'].health.orientation)
	...
	self:RegisterEvent('ACTIVE_TALENT_GROUP_CHANGED', UF.UpdateClickSet)
...
function UF:Update_RaidHeader(header, db, isForced)
	...
	if not headerHolder.positioned then
		...
		E:CreateMover(headerHolder, headerHolder:GetName()..'Mover', L['Raid Frames'], nil, nil, nil, 'ALL,RAID', function() return E.db.unitframe.units['raid'].enable; end)			
	...
	do
		...
		if USE_POWERBAR then
			...
			elseif USE_MINI_POWERBAR then
				...
				power:Point("CENTER", frame, "BOTTOM", 0, BORDER + (POWERBAR_HEIGHT/2))
