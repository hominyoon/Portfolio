...
function UF:Construct_PartyFrames(unitGroup)
	...
	if self.isChild then
		...
		self.Health = UF:Construct_HealthBar(self, true, true, 'RIGHT', E.db.unitframe.units.party.health.orientation)
		...
		self.Castbar = UF:Construct_Castbar(self, 'RIGHT')
...
function UF:Update_PartyHeader(header, db)	
	...
	if not headerHolder.positioned then
		...
		E:CreateMover(headerHolder, headerHolder:GetName()..'Mover', L['Party Frames'], nil, nil, nil, 'ALL,PARTY,ARENA', function() return E.db.unitframe.units.party.enable; end)
