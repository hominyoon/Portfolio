...
function UF:Construct_BossFrames(frame)	
	...
	frame.Castbar = self:Construct_Castbar(frame, 'RIGHT', frame:GetName()..' '..L['Castbar'])
	frame.RaidIcon = UF:Construct_RaidIcon(frame)
	frame.AltPowerBar = self:Construct_AltPowerBar(frame)
	frame.Range = UF:Construct_Range(frame)
	
	BossHeader:Point('BOTTOMRIGHT', E.UIParent, 'RIGHT', -105, -165) 
	E:CreateMover(BossHeader, BossHeader:GetName()..'Mover', L['Boss Frames'], nil, nil, nil, 'ALL,PARTY,RAID', function() return E.db.unitframe.units.boss.enable; end)
end
...
function UF:Update_BossFrames(frame, db)
	...
	if INDEX == 1 then
		...
		if db.growthDirection == 'UP' then
			frame:Point('BOTTOMRIGHT', _G['ElvUF_Boss'..INDEX-1], 'TOPRIGHT', 0, db.space + db.castbar.height)
		else
			frame:Point('TOPRIGHT', _G['ElvUF_Boss'..INDEX-1], 'BOTTOMRIGHT', 0, -(db.space + db.castbar.height))
		end
	end	

	BossHeader:Width(UNIT_WIDTH)
	BossHeader:Height(UNIT_HEIGHT + (UNIT_HEIGHT + db.space + db.castbar.height) * 3)
