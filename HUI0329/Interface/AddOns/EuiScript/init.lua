...
function S:ModifyChatSticky()
	...
	if not E.db.chat.sticky then return; end
...
--	local position = MinimapMover:GetPoint()
	...
	for i = 1, #buttons do
		...
		--		buttons[i]:SetPoint("TOPRIGHT", buttons[1], "TOPLEFT", -1, 0)
			elseif i > 1 then
				buttons[i]:SetPoint("TOP", button, "BOTTOM", 0, -maxbuttonsize*(i-1)-i)
			end
		else
			if i == 1 then
		...
		--		buttons[i]:SetPoint("TOPRIGHT", buttons[1], "TOPLEFT", -1, 0)
				...
				buttons[i]:SetPoint("BOTTOM", button, "TOP", 0, maxbuttonsize*(i-1)+i)
			end
		end
...
function S:FixPriestTicks()
	...
	E.global.unitframe.ChannelTicks[GetSpellInfo(47540)] = penanceTicks --Penance
	G.unitframe.ChannelTicks[GetSpellInfo(47540)] = penanceTicks
end
...
--ObjectiveTrackerFrame.HeaderMenu.MinimizeButton
	...
	accept:SetPoint("RIGHT", ObjectiveTrackerFrame.HeaderMenu.MinimizeButton, "LEFT", -(accept.text:GetWidth()+5), 0)
	accept:SetScript("OnClick", function(self)
		E.db["euiscript"].idq = not E.db["euiscript"].idq;
	--	self:SetChecked(E.db["euiscript"].idq);
	end)
	accept:SetChecked(E.db["euiscript"].idq);
--	E:GetModule('Skins'):HandleCheckBox(accept, true)
end
...
local function CreatePOI()
	...
	f.texture:SetVertexColor(E.db.euiscript.poiColor.r, E.db.euiscript.poiColor.g, E.db.euiscript.poiColor.b)
...
function S:ToggleEuiScriptPoi()
	if E.db.euiscript.poi then
		...
		EuiScriptPOI.texture:SetVertexColor(E.db.euiscript.poiColor.r, E.db.euiscript.poiColor.g, E.db.euiscript.poiColor.b)
