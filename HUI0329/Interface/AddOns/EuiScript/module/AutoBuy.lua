...
----------------------------------------------------------------------------------------
local ItemIDPattern = "item:(%d+):%d+:%d+:%d+:%d+:%d+:%-?%d+:%-?%d+:%d+:%d+:%d+"
...
local function BuyReagents()
	...
	for i = 1, GetMerchantNumItems() do
		...
		if ItemLink then
			ItemID = tostring(select(3, find(ItemLink, ItemIDPattern)))
		end
...
f:SetScript("OnEvent", function(self, event)
	...
	if event == "MERCHANT_CLOSED" or event == "PLAYER_ENTER_WORLD" then
		buyed = false
	end
end)