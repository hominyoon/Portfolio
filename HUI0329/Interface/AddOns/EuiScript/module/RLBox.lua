...
local OFFLINETEXTURE = "Interface\\TargetingFrame\\UI-RaidTargetingIcons";
...
local function GetUnitFromName(name, guid)
	if IsInRaid() then
...
local function OnMouseDown(self, button)
	if button == "LeftButton" then
		...
		SendChatMessage(sformat("EUI:%s: %s", (self.partytext:GetText() or ' ')..(self.left:GetText() or ' ')..'-'..(self.spellname or ' '), self.right:GetText() or ' '), channel)
...
local function PostionBar(anchor, DB)
	if anchor and anchor.Bars and #anchor.Bars > 0 then
		if R.db.party then
			table.sort(anchor.Bars, function(a, b)
		...
		local perrow = math.ceil(1 + #anchor.Bars /2)
...
local function SendCooldownInfo(partyno, wpTime)
	local anchor = _G['EUIRaidToolBox1']
	if anchor.Bars and #anchor.Bars > 0 then
		for i = 1, #anchor.Bars -1 do
			...
			local msg = string.format(WHISPERMSG, partyno, wpTime)
...
local function iconOnUpdate(self)
	if self.endTime and self.duration then
		...
		if (time > self.endTime) then
			...
			local remaining = sformat('%d', self.endTime - time)
	...
	if unit and (UnitIsDeadOrGhost(unit) or not UnitIsConnected(unit)) then
		if not self.curtex:find('RaidTargetingIcons') then
	...
	if self.curtex:find('RaidTargetingIcons') then
	--	E:StopFlash(self)
		E:Flash(self, 1.2)
	else
		E:StopFlash(self)
...
function R:CreateIconBar(parent, id, DB)
	if _G["EUIRLBox"..parent:GetName().."Icon"..id] then
		...
		if icon.add then icon.add:Size(math.floor(DB.IconSize/2)) end
		if icon.sut then icon.sut:Size(math.floor(DB.IconSize/2)) end
	...
	if R.db.party then
		...
		icon.add:Size(math.floor(DB.IconSize/2))
		icon.add:Point('BOTTOMRIGHT', icon.bar, 'TOPRIGHT', -math.floor(DB.IconSize/2), 4)	
		...
		icon.sut:Size(math.floor(DB.IconSize/2))
...
local function barOnUpdate(self)
	...
	if (time > self.endTime) then
		...
		if not icon.sendready then
			if icon.EndChannel and icon.EndChannel ~= 'NONE' then
				...
				SendChatMessage(sformat(readyMsg, icon.partytext:GetText()..icon.playername, icon.spellid, icon.spellname), icon.EndChannel)
			end
			icon.sendready = true
			icon.sendcast = false
		end
	...
	--	E:StopFlash(icon)
		...
		if R.db.UIType == 'icon' then
			if (not icon.duration) or (icon.duration and icon.duration < 0) then 
...
function R:COMBAT_LOG_EVENT_UNFILTERED(...)
	...
	if spellID == 61391 then spellID = 132469 end 
					...
					if icon.cds and icon.cds > 0 then
						...
						if not icon.sendcast then
							...
							if channel and channel ~= 'NONE' then
								if channel == 'RAID' and IsPartyLFG() then channel = 'INSTANCE_CHAT' end							
								SendChatMessage(sformat(castMsg, icon.partytext:GetText()..icon.playername, spellID, icon.spellname, icon.duration or NONE), channel)
							end
							icon.sendcast = true;
