...
fr:RegisterEvent("PLAYER_ENTER_WORLD")
fr:SetScript("OnEvent", function(self,event)
	if event == 'PLAYER_ENTER_WORLD' then
		self:UnregisterEvent('PLAYER_ENTER_WORLD')
...
local function InfoFilter(IFSself, IFSevent, IFSmsg, IFSauthor, _, _, _, IFSflag, _, _, IFSchannel, _, IFSid, guid)
	...
	if (IFSmatch >= E.global.InfoFilter.matchNum) then		
		...
		if sameUser[IFSauthor] then
			local findIt = false;
			if (sameUser[IFSauthor].memo1 == IFSmsg or sameUser[IFSauthor].memo2 == IFSmsg) and (GetTime() - sameUser[IFSauthor].time) < E.global.InfoFilter.repeatTime then
				if not cacheBlackName[IFSauthor] then
					cacheBlackName[IFSauthor] = 1;
				elseif cacheBlackName[IFSauthor] < 15 then 
					cacheBlackName[IFSauthor] = cacheBlackName[IFSauthor] + 1;
			...
			elseif sameUser[IFSauthor].id == 1 then
				sameUser[IFSauthor].memo1 = IFSmsg;
				sameUser[IFSauthor].id = 2;
			elseif sameUser[IFSauthor].id == 2 then
				sameUser[IFSauthor].memo2 = IFSmsg;
				sameUser[IFSauthor].id = 1;
			end
		else
			sameUser[IFSauthor] = {['time'] = GetTime(),['memo1'] = IFSmsg, ['id'] = 1,};
		end
		return false;
	end
end
...
do
	...
	elseif L == "koKR" then
		whisp = "|cFF33FF99EUI|r: 최소한 레벨에 도달해야 %d 레벨 귓속말 대화가 가능합니다。"
