...
--[[	DBM_Core = {
		...
		ThogarAssist = {
			order = 60,
			type = 'execute',
			name = L["Enable/Disable"]..L["ThogarAssist"],
			func = function() 
				if E:IsConfigurableAddOn('ThogarAssist') then
					E:Print(L['Disabled']..L["ThogarAssist"]);
					DisableAddOn('ThogarAssist');
					E:StaticPopup_Show("CONFIG_RL");
				else
					E:Print(L["Enabled"]..L["ThogarAssist"]);
					EnableAddOn('ThogarAssist');
					E:StaticPopup_Show("CONFIG_RL");
				end
			end,
		},
		ThogarAssistConfigFrame = {
			order = 61,
			type = 'execute',
			name = L['ThogarAssist Config Frame'],
			disabled = function() return not IsAddOnLoaded('ThogarAssist'); end,
			func = function()
				if E:IsConfigurableAddOn('ThogarAssist') then
