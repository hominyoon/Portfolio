...
--	local statusBar = CreateFrame("StatusBar", nil, frame)
	...
	spark:SetPoint('CENTER', statusBar:GetStatusBarTexture(), (E.db.general.transparent and E.db.general.transparentStyle == 2) and 'LEFT' or 'RIGHT')
...
local function Update(self, event, unit)
	...
	if(auraBars.forceShow) then
		for index = 1, auraBars.maxBars do	
			...
			auras[lastAuraIndex].value = 25716
		...
		for index = 1, 40 do
			local name, rank, icon, count, debuffType, duration, expirationTime, unitCaster, isStealable, shouldConsolidate, spellID, _, _, _, v1, v2 = UnitAura(unit, index, helpOrHarm)
			...
			elseif v1 and v2 then
				spellValue = (v1 > v2) and v1 or v2
			end
			if spellValue and spellValue < 1000 then spellValue = nil end
			
			local Unit = unit;
			if unit == 'vehicle' or unit == 'pet' then Unit = 'player' end
			if not E.db.unitframe.units[Unit].aurabar.showValue then spellValue = nil end
