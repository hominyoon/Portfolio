Skada:AddLoadableModule("Healing", function(Skada, L)
		...
		if player then
			...
			if set == Skada.current and amount > 0 then
				...
				if not player.healed[heal.dstName] then
					...
					player.healed[heal.dstName] = {name = heal.dstName, shortname = heal.dstName, class = class, role = role, amount = 0, spell = {}}
				end
	...
	local function SpellAbsorbed(timestamp, eventtype, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, ...)
		...
		if aspellId == 115069 or aspellId == 114556 then return end--ignore Stance of the Sturdy Ox, Purgatory
		local valid = band(aFlags, RAID_FLAGS) ~= 0 or (band(aFlags, PET_FLAGS) ~= 0 and Skada:IsValidPet(aGUID)) or (Skada:IsValidPlayer(aGUID) and band(aFlags, COMBATLOG_OBJECT_CONTROL_PLAYER) ~= 0)
		if valid then
	...
	local function AuraRefresh(timestamp, eventtype, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, spellId, spellName, spellSchool, auraType, amount)
		if amount ~= nil then
			if shields[dstGUID] and shields[dstGUID][spellId] and shields[dstGUID][spellId][srcGUID] then
				local prev = shields[dstGUID][spellId][srcGUID]
				if prev and prev > amount and Skada.current then
	...
	local function spell_tooltip(win, id, label, tooltip)
		...
		if player then
			...
			if spell and (spell.hits > 0) then
				...
				tooltip:AddLine(player.shortname.." - "..label)
	...
	function modHealed:Enter(win, id, label)
		...
		win.modedata.playername = player.shortname
	...
	function modHealedSpell:Enter(win, id, label)
		...
		self.title = L["Healed"]..": "..win.modedata.playername.." > "..label
	end
	...
	function total:Update(win, set)
		...
		for _, player in pairs(set._playeridx) do
			if (player.healing > 0) or (player.overhealing > 0) then
				...
				dataTable[tnr] = {id = player.id, healing = player.healing, total = playerTotal, name = player.shortname, perc = mypercent, class = player.class, role = player.role}
				tnr = tnr +1
			end
		end
	...
	function totalSpell:Enter(win, id, label)
		...
		local player = Skada:find_player(win:get_selected_set(), id)
		if player then
			self.title = L["Total healing"]..": "..player.shortname
		end
	end
	...
	function overhealSpell:Enter(win, id, label)
		win.modedata.playerid = id
		self.title = L["Overhealing"]..": "..label
	end
	...
	function absorbedSpell:Enter(win, id, label)
		win.modedata.playerid = id
		self.title = L["Absorbed heal"]..": "..label
	end
	...
	function takenSpell:Enter(win, id, label)
		...
		self.title = L["Healing taken"]..": "..label
	end
	...
	function mod:OnEnable()
		...
		--Skada:RegisterForCL(AuraRefresh, 'SPELL_AURA_REFRESH', {src_is_interesting_nopets = true}, true)
		--Skada:RegisterForCL(AuraRemoved, 'SPELL_AURA_REMOVED', {src_is_interesting_nopets = true}, true)

		-- handlers for mob healings
		Skada:RegisterForCL(mobHealing, 'SPELL_HEAL', {src_is_not_interesting = true, dst_is_not_interesting = true})
		Skada:RegisterForCL(mobHealing, 'SPELL_PERIODIC_HEAL', {src_is_not_interesting = true, dst_is_not_interesting = true})
